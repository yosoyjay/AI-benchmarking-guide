...
class TestParseFlashAttentionOutput:
    ...
    def test_unmatched_input_returns_empty(self, caplog):
        rows = parse_flash_attention_output("no matching content here\n")
        assert rows == []

    def test_warns_on_no_matches(self, caplog):
