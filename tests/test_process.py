...
def _make_ctx(tmp_path, benchmark="test_bench", **extra):
...
class TestRowHelper:
    def test_builds_all_columns(self, tmp_path):
        ctx = _make_ctx(tmp_path)
        row = _row(ctx, "tflops", 42.5, "TFLOPS", {"m": 1024})
        assert set(row.keys()) == set(CSV_COLUMNS)

    def test_metadata_is_json_string(self, tmp_path):
        ctx = _make_ctx(tmp_path)
    ...
    def test_metric_value_is_string(self, tmp_path):
        ctx = _make_ctx(tmp_path)
...
class TestGemmCublasLtToCsv:
    def test_two_rows_per_input(self, tmp_path):
    ...
    def test_metadata_includes_datatype(self, tmp_path):
...
class TestRcclBandwidthToCsv:
    def test_includes_algo(self, tmp_path):
...
class TestFlashAttentionToCsv:
    def test_two_metrics_per_input(self, tmp_path):
...
class TestHbmBandwidthToCsv:
    def test_three_metrics_per_op(self, tmp_path):
...
class TestCpuStreamToCsv:
    def test_uses_gbs_unit(self, tmp_path):
...
class TestProcessRun:
    def test_writes_csv_and_combined(self, tmp_path):
    ...
    def test_combined_accumulates(self, tmp_path):
        ctx = _make_ctx(tmp_path, "test_bench")
