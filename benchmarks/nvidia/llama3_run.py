...
class LLAMA3Pretraining:
    ...
    def run(self, ctx=None):
        ...
        logger.info(
            "Pulling and launching NeMo docker container for %s and logging at 'Outputs/log.txt'.", self.machine_name
        )
        ...
        command = [
            ...
            "stack=67108864",
            "-v",
            f"{self.mount_path}:/workspace/nemo-run",
            self.container,
            "bash",
            "-c",
            f"cd /workspace/nemo-run && python {self.training_script} --model_size {self.model_size} --machine_name {(self.machine_name.split() or [self.machine_name])[-1]}",
        ]
        ...
        if ctx is not None:
            ...
            ctx.extra["model_size"] = self.model_size
            return time_ss, loss_ss
        ...
        time_ss, loss_ss = self.plot_results(log_path)
        ...
        tools.export_markdown(
            f"LLAMA3 {self.model_size} Pretraining Summary",
            f"Pretraining results for LLAMA3 {self.model_size} on {self.machine_name}.",
            table,
        )
